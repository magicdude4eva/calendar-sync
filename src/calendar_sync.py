...
def main():
    ...
    parser.add_argument(
        ...
        dest="import_events",
        action="store_true",
        help="Import events from ICS feeds",
    )
    parser.add_argument(
        "--cleanup",
        nargs="?",
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Simulate import without modifying calendar",
    )
    parser.add_argument(
        "--config",
        default="config.json",
    ...
    client = DAVClient(
        config["caldav_url"], username=config["username"], password=config["password"]
    )
    ...
    if args.import_events:
        ...
        for feed in config["ics_feeds"]:
            ...
            existing_uids.update(
                get_existing_events(
                    calendar, feed_prefix, cleanup_mode=False, all_events=all_events
                )
            )
        ...
        extra_events = compute_extra_events(
            config, config["uid_prefix"], tz, now, until_date
        )

        for event in extra_events:
            if event.uid in existing_uids:
                logger.info(
                    f"⏭️ Skipping extra event '{event.name}' (UID {event.uid}) – already exists."
                )
                continue
            if args.dry_run:
                logger.info(
                    f"💡 [Dry-run] Would create extra event '{event.name}' on {event.begin} (UID {event.uid})"
                )
            else:
                try:
                    calendar.save_event(event.serialize())
                    logger.info(
                        f"✅ Created extra event '{event.name}' on {event.begin}"
                    )
